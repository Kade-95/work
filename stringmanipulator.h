...
void clearscreen(){
    ...
    char buff[] = "     ";
    ::WriteConsoleA(hOutput, buff, 0, NULL, NULL);
}
...
bool isCapitalAlpha(char value){
...
bool isNumberString(string value){
...
int strTOin(string value){
    int rValue= atoi(value.c_str());
    return rValue;
}

string stringReplacer(string value, string from, string *to){
    size_t pos = 0;
    int i = 0;
...
bool searchString(string value, string toFind){
...
vector<string> rowTOstring(MYSQL_ROW row, MYSQL_RES *res){
    vector<string> value;
    stringstream oss;
...
bool isSpaceString(string value){
    for(int i=0; i<value.length(); i++){
        if(!isspace(value[i])){
            return false;
        }
...
bool isEqual(string value, string check){
